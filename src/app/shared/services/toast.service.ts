  ...
  public HandelDbMessages(messages?: Message[]): boolean {
    if (messages && messages.length > 0) {
      ...
      if (messageType == 1) {
        ...
        if (this.translate.currentLang == 'en') {
          this.toastr.error(messages[0].msgBodyEn, messages[0].msgHeaderEn, {
            closeButton: true,
            positionClass: 'toast-top-right'
          });
        }
        ...
        if (this.translate.currentLang == 'ar') {
          this.toastr.error(messages[0].msgBodyAr, messages[0].msgHeaderAr, {
            closeButton: true,
            positionClass: 'toast-top-left'
          });
        }
        ...
        if (!window.location.pathname.includes('/portal/'))
          if (messages[0].msgID !== 5 && messages[0].msgID !== 6) {
      ...
      return (messages[0].msgID == 5 || messages[0].msgID == 44 || messages[0].msgID == 6) ? true : false;
    } else {
      return true;
    }
  }
